...
export async function POST(request: Request, { params }: Params) {
  ...
  if (!ALLOWED_ONBOARDING_MIME_TYPES.has(mimeType)) {
    return NextResponse.json({ error: "File type not allowed" }, { status: 400 });
  }

  const storagePath = buildOnboardingStoragePath(resolved.invite.id, fieldId, file.name);
  ...
  try {
    ...
    if (resolved.invite.status === "pending") {
      await supabase.from("onboarding_invites").update({ status: "in_progress" }).eq("id", resolved.invite.id);
    }

    const { error: uploadError } = await supabase.storage.from(ONBOARDING_BUCKET).upload(storagePath, buffer, {
